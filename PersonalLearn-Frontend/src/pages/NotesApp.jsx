...
function NotesApp({ topic }) {
  ...
  useEffect(() => {
    if (topic && topic !== currentFolder) {
      switchFolder(topic);
    }
  }, [topic, currentFolder, switchFolder]);
  ...
  // Reset search state when page search query changes
  const handlePageSearchChange = useCallback((query) => {
